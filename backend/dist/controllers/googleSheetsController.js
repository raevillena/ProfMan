...
class GoogleSheetsController {
    async createGradebook(req, res) {
        try {
            ...
            const result = await googleSheetsService.createGradebookSpreadsheet(userId, branchId, branchTitle, students);
            if (result.success) {
                res.status(200).json({
                    success: true,
                    data: {
    ...
    async updateScores(req, res) {
        try {
            ...
            const result = await googleSheetsService.updateGradebookScores(userId, spreadsheetId, studentScores);
            if (result.success) {
                res.status(200).json({
                    success: true,
    ...
    async addQuizResults(req, res) {
        try {
            ...
            const result = await googleSheetsService.addQuizResults(userId, spreadsheetId, quizTitle, results);
            if (result.success) {
                res.status(200).json({
                    success: true,
    ...
    async getSpreadsheetData(req, res) {
        try {
            ...
            const result = await googleSheetsService.getSpreadsheetData(userId, spreadsheetId, range);
            if (result.success) {
                res.status(200).json({
                    success: true,
